...
The Discord (`dsqrd`) and Slack (`slqs`) Wayland clients are installed system-wide via the NixOS config at `~/.config/nixos/flake.nix`, which pins them as flake inputs (NOT from any local checkout — pulling `~/repos/dsqrd` does nothing to the running system). **dsqrd now tracks my fork `github:marsa099/dsqrd` (main), NOT daphen** — the fork adds features daphen declined (e.g. the Copilot catch-up button + `dsqrd-cli`); `slqs` still tracks `github:daphen/slqs`. To update: `update-dsqrd` (or `update-dsqrd slqs`) — a `~/.scripts/` helper that bumps the flake lock, runs `sudo nixos-rebuild switch --flake ~/.config/nixos`, then restarts the long-running daemon + Quickshell UI (a rebuild swaps the binary but won't restart already-running processes). **dsqrd's update is two-legged** (it's a fork): `update-dsqrd` first merges daphen's `origin/main` into the fork's `main` (auto if clean; on conflict it launches `claude --model claude-opus-4-8` in `~/repos/dsqrd` to resolve, needs an interactive terminal), pushes, then bumps+rebuilds. The in-app "update available" badge is driven by the daemon (`dsqrd.py`) polling GitHub — it now checks the fork (`DSQRD_UPDATE_REPO`) for a stale build AND daphen (`DSQRD_UPSTREAM_REPO`) for unmerged upstream commits, both baked in `flake.nix`. `update-dsqrd --check` reports both legs (exit 10 = update available). Both clients run a headless daemon (`dsqrd.py` / `slqs` binary) plus a `qs -p .../share/<app>/ui` UI process; the `*-client` wrapper starts the daemon then execs the UI.
