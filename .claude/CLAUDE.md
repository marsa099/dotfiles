...
Media viewing (`v`): daphen replaced the old 150-line `media-viewer.sh` with a thin shim that prefers a `mediactl view` binary on PATH (`6fa4907`, then `7fc5aee` added an inline imv/mpv fallback so it is portable). `mediactl` is **not** in nixpkgs and no public repo implements that interface — it is only on daphen's machine. The fallback works but is stripped: no theme-matched imv background, no output-relative window sizing, `mix` collapsed into `video` (loses `h`/`l` stepping), and no URL content-type sniffing (giphy/tenor embeds open in the browser). To get the good behaviour back, drop the pre-`6fa4907` script (`git show 6fa4907^:media-viewer.sh`) into `~/.scripts/mediactl` behind a `view` subcommand — the shim's `command -v mediactl` check runs first, so that is a supported extension point that survives upstream pulls.
