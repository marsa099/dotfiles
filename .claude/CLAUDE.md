...
The Discord (`dsqrd`) and Slack (`slqs`) Wayland clients are installed system-wide via the NixOS config at `~/.config/nixos/flake.nix`, which pins them as flake inputs (NOT from any local checkout — pulling `~/repos/dsqrd` does nothing to the running system). **All three track daphen directly** (`github:daphen/dsqrd`, `github:daphen/slqs`, `github:daphen/mlqs`) — dsqrd went back to upstream on 2026-08-10, mlqs on 2026-08-12 (IMAP vendor merged upstream as PR #3; the smart-threading fork branch is archived locally in `~/repos/mlqs`). To update: `update-dsqrd` (or `update-dsqrd slqs` / `mlqs`) — a `~/.scripts/` helper that bumps the flake lock, runs `sudo nixos-rebuild switch --flake ~/.config/nixos`, then restarts the long-running daemon + Quickshell UI (a rebuild swaps the binary but won't restart already-running processes). All three are single-leg since 2026-08-12 (the script's fork machinery is kept but unused). Gotcha: after editing a flake input's URL, the script reports "up to date" without rebuilding — `nix flake metadata` re-locks the changed input on the fly — so rebuild manually after a repo repoint. The in-app "update available" badge is the daemon polling daphen's main SHA. `update-dsqrd --check` reports it (exit 10 = update available). Both clients run a headless daemon (`dsqrd.py` / `slqs` binary) plus a `qs -p .../share/<app>/ui` UI process; the `*-client` wrapper starts the daemon then execs the UI.

`dsqrd-cli` (used by the `dsqrd` skill) is fork-only and is **not** packaged — it runs from `~/repos/dsqrd/dsqrd-cli` and talks to whatever daemon is up, so a stock daphen build serves it fine (every command it uses — `history`, `send`, `open`, `summarize` — is upstream). Keep `~/repos/dsqrd` around even though the system no longer builds from it. Upstream carries one open bug the fork had fixed: the `dsqrd-client` launcher's aliveness check greps a bare `dsqrd\.py`, so any unrelated process with that filename in its argv (an editor, an agent prompt) makes it skip starting the daemon and the UI comes up empty on a dead socket — fix pending as daphen/dsqrd PR #11. Note this also bites *you*: never `pkill -f 'share/dsqrd/dsqrd\.py'` from an inline `bash -c`, since the pattern matches your own shell's command line and kills the session.
