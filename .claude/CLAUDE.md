...
The supported extension point is now an **env var**: `8cae084` changed the launcher to `export SLK_MEDIA_VIEWER="''${SLK_MEDIA_VIEWER:-<pkg>/share/dsqrd/media-viewer.sh}"`, so a value already in the environment wins. To restore the good behaviour, save the pre-`6fa4907` script (`git -C ~/repos/dsqrd show 6fa4907^:media-viewer.sh`) somewhere stable and point `SLK_MEDIA_VIEWER` at it via `environment.sessionVariables` in the NixOS config. It needs `imv`, `mpv`, `jq`, `curl`, `niri msg`, and ImageMagick `identify` on PATH, and reads `~/.config/theme_mode` + `~/.config/themes/colors.json` for the background colour. The same var is read by the QML UI (`ui/Backend.qml`), whose own fallback path is `~/.config/qs-chat-clients/media-viewer.sh` — but the launcher always sets the var, so that fallback never fires in practice.
