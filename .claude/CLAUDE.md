...
The Discord (`dsqrd`) and Slack (`slqs`) Wayland clients are installed system-wide via the NixOS config at `~/.config/nixos/flake.nix`, which pins them as flake inputs (NOT from any local checkout — pulling `~/repos/dsqrd` does nothing to the running system). **Both track daphen directly** (`github:daphen/dsqrd`, `github:daphen/slqs`) — dsqrd went back to upstream on 2026-08-10 once daphen shipped his own `open` deep-link command; only `mlqs` is still built from a marsa099 fork. To update: `update-dsqrd` (or `update-dsqrd slqs` / `mlqs`) — a `~/.scripts/` helper that bumps the flake lock, runs `sudo nixos-rebuild switch --flake ~/.config/nixos`, then restarts the long-running daemon + Quickshell UI (a rebuild swaps the binary but won't restart already-running processes). Only `mlqs` takes the two-legged fork path (merge daphen's `upstream/main` into the fork first, `pi` resolves conflicts); dsqrd and slqs are single-leg. The in-app "update available" badge is the daemon polling daphen's main SHA. `update-dsqrd --check` reports it (exit 10 = update available). Both clients run a headless daemon (`dsqrd.py` / `slqs` binary) plus a `qs -p .../share/<app>/ui` UI process; the `*-client` wrapper starts the daemon then execs the UI.
